...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin, AutomaticKeepAliveClientMixin {
  ...
  void _startFlashsaleTimer() {
    ...
    _flashsaleTimer = Timer.periodic(period, (timer) {
      ...
      if (_flashsaleSecond == 0) {
        ...
        Fluttertoast.showToast(msg: AppLocalizations.of(context)!.translate('flash_sale_is_over')!, toastLength: Toast.LENGTH_LONG);
      }
    });
  }
  ...
  void initState() {
    ...
    _homeBannerBloc.add(GetHomeBanner(sessionId: SESSION_ID, apiToken: apiToken));

    _flashsaleBloc = BlocProvider.of<FlashsaleBloc>(context);
    _flashsaleBloc.add(GetFlashsaleHome(sessionId: SESSION_ID, skip: '0', limit: '8', apiToken: apiToken));

    _lastSearchBloc = BlocProvider.of<LastSearchBloc>(context);
    _lastSearchBloc.add(GetLastSearchHome(sessionId: SESSION_ID, skip: '0', limit: '8', apiToken: apiToken));

    _homeTrendingBloc = BlocProvider.of<HomeTrendingBloc>(context);
    _homeTrendingBloc.add(GetHomeTrending(sessionId: SESSION_ID, skip: '0', limit: '8', apiToken: apiToken));

    _categoryForYouBloc = BlocProvider.of<CategoryForYouBloc>(context);
    _categoryForYouBloc.add(GetCategoryForYou(sessionId: SESSION_ID, apiToken: apiToken));
    ...
    _recomendedProductBloc.add(GetRecomendedProduct(sessionId: SESSION_ID, skip: _apiPageRecomended.toString(), limit: LIMIT_PAGE.toString(), apiToken: apiToken));
                      ...
                      delegate: SliverChildListDelegate(
                          [
                            ...
                            Container(
                              ...
                              child: Text(AppLocalizations.of(context)!.translate('flash_sale')!, style: GlobalStyle.sectionTitle),
                            ),
                            Container(
                                ...
                                children: [
                                  ...
                                  Expanded(
                                    child: GestureDetector(
                                      ...
                                      child: Text(AppLocalizations.of(context)!.translate('view_all')!, style: GlobalStyle.viewAll, textAlign: TextAlign.end),
                                    ),
                                  )
                                ],
                              ),
                                ...
                                builder: (context, state) {
                                  if(state is FlashsaleHomeError) {
                                    ...
                                    if(_lastDataFlashsale){
                                      return Container(
                                          child: Center(
                                              child: Text(AppLocalizations.of(context)!.translate('no_flash_sale')!, style: TextStyle(
                                                  fontSize: 14,
                                                  color: BLACK_GREY
                                              ))
                                          )
                                      ...
                                      if(flashsaleData.length==0){
                                        return _shimmerLoading.buildShimmerHomeFlashsale(boxImageSize);
                                      } else {
                                        return ListView.builder(
                                          ...
                                          itemBuilder: (BuildContext context, int index) {
                                            if(index == flashsaleData.length){
                                              return _buildLastBox(boxImageSize, FlashSalePage());
                                            } else {
                                  ...
                                  if(state is HomeTrendingError) {
                                    ...
                                    if(_lastDataTrending){
                                      ...
                                      if(homeTrendingData.length==0){
                                        return _shimmerLoading.buildShimmerTrending(MediaQuery.of(context).size.width/2);
                                      } else {
                            ...
                            Container(
                              ...
                              child: Row(
                                ...
                                children: [
                                  Text(AppLocalizations.of(context)!.translate('your_last_search')!, style: GlobalStyle.sectionTitle),
                                  GestureDetector(
                                    onTap: (){
                                ...
                                builder: (context, state) {
                                  if(state is LastSearchHomeError) {
                                    ...
                                    if(_lastDataSeach){
                                      return Container(
                                          child: Center(
                                              child: Text(AppLocalizations.of(context)!.translate('no_last_search')!, style: TextStyle(
                                                  fontSize: 14,
                                                  color: BLACK_GREY
                                              ))
                                          )
                                        ...
                                        return ListView.builder(
                                          ...
                                          itemBuilder: (BuildContext context, int index) {
                                            if(index == lastSearchData.length){
                                              return _buildLastBox(boxImageSize, LastSearchPage());
                                            } else {
                                              return _globalWidget.buildHorizontalProductCard(context, lastSearchData[index]);
                                            }
                                          },
                                        );
                                      }
                            ...
                            Container(
                              margin: EdgeInsets.only(top:30, left: 16, right: 16),
                              child: Text(AppLocalizations.of(context)!.translate('category_for_you')!, style: GlobalStyle.sectionTitle),
                            ),
                            _createCategoryForYou(boxImageSize, categoryForYouHeightShort, categoryForYouHeightLong),
                            Container(
                              margin: EdgeInsets.only(top:30, left: 16, right: 16),
                              child: Text(AppLocalizations.of(context)!.translate('recomended_product')!, style: GlobalStyle.sectionTitle),
                            ),
                              ...
                              builder: (context, state) {
                                if (state is RecomendedProductError) {
                                  ...
                                  if (_lastDataRecomended && _apiPageRecomended == 0) {
                                    return Container(
                                        child: Text(AppLocalizations.of(context)!.translate('no_recomended_product')!,
                                            style: TextStyle(
                                    ...
                                    if (recomendedProductData.length == 0) {
                                      ...
                                      return CustomScrollView(
                                          ...
                                          slivers: <Widget>[
                                            SliverPadding(
                                              ...
                                              sliver: SliverGrid(
                                                ...
                                                delegate: SliverChildBuilderDelegate(
                                                    ...
                                                    return _globalWidget.buildProductGrid(context, recomendedProductData[index]);
                                                  },
                                            ...
                                            SliverToBoxAdapter(
                                              child: (_apiPageRecomended==LIMIT_PAGE && recomendedProductData.length < LIMIT_PAGE)?Wrap():_globalWidget.buildProgressIndicator(_lastDataRecomended),
                                            ),
                                          ]);
                                    }
                                  }
        ...
        if(state is HomeBannerError) {
          ...
          if(_lastDataHomeBanner){
            return Container(
                ...
                height: homeBannerHeight,
                child: Center(
                    child: Text(AppLocalizations.of(context)!.translate('no_home_banner_data')!, style: TextStyle(
                        fontSize: 14,
                        color: BLACK_GREY
                    ))
                )
            ...
            if(homeBannerData.length==0){
              return _shimmerLoading.buildShimmerHomeBanner(homeBannerWidth, homeBannerHeight);
            } else {
              return Column(
                children: [
                  CarouselSlider(
        ...
        child: GestureDetector(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              SizedBox(height: 10,),
              Text(AppLocalizations.of(context)!.translate('check_another_product')!, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold), textAlign: TextAlign.center,)
            ],
          ),
        ),
      ),
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 4),
                      Text(homeTrendingData[index].sale+' '+AppLocalizations.of(context)!.translate('product')!, style: TextStyle(
        ...
        if(state is CategoryForYouError) {
          ...
          if(_lastDataCategoryForYou){
            return Container(
                ...
                height: categoryForYouHeightLong,
                child: Center(
                    child: Text(AppLocalizations.of(context)!.translate('no_category_for_you')!, style: TextStyle(
                        fontSize: 14,
                        color: BLACK_GREY
                    ))
                )
            ...
            if(categoryForYouData.length==0){
              return _shimmerLoading.buildShimmerCategoryForYou(categoryForYouHeightShort*3, categoryForYouHeightLong);
            } else {
              return Container(
